"""Unitree Go2 HIM velocity environment configurations.

Mirrors the G1 HIM pattern exactly:
- Actor uses three HIM observation groups (proprio_history / proprio_current /
  estimator_vel) — no height scan, fully blind proprioceptive policy.
- Critic inherits the full privileged observation group from the base Go2 config.
- All Go2-specific customisations (sensors, reward stds, termination, body names)
  are preserved by calling the base Go2 env configs and only replacing the
  observation layout.

Go2 single-step proprioceptive dim S = 47:
...
def _proprio_terms() -> dict[str, ObservationTermCfg]:
  """Go2 proprioceptive single-step terms (identical structure to G1 HIM).

  Noise scales match the base velocity task actor terms.
  """
  return {
...
def unitree_go2_him_flat_env_cfg(play: bool = False) -> ManagerBasedRlEnvCfg:
  """Unitree Go2 flat terrain HIM velocity configuration."""
  cfg = unitree_go2_flat_env_cfg(play=play)
  cfg.observations = _him_observations(cfg, play)
  return cfg


def unitree_go2_him_rough_env_cfg(play: bool = False) -> ManagerBasedRlEnvCfg:
  """Unitree Go2 rough terrain HIM velocity configuration.
